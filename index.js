...
app.use((req, res, next) => {
    console.log("Eu sou um mildewares");
    next();
})
    ...
    Categoria.findOne({ slug: req.params.slug }).then((categoria) => {
        if (categoria) {
            Postagem.find({ categoria: categoria._id }).then((postagens) => {
                res.render("categoria/postagem", { postagens: postagens, categoria: categoria })
            }).catch((err) => {
        ...
        req.flash("error_msg", "Houve um erro interno ao carregar a pagina dessa categori")
        res.redirect("/");
    })
})
